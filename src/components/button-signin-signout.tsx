...
//   const userIsLoggedIn = true; //better-auth
...
export function ProfileDropdown({ isLoggedIn }: { isLoggedIn: boolean }) {
  if (!isLoggedIn) {
    // если не залогинен → показываем Sign In
    return (
      <Dialog>
        <DialogTrigger asChild>
          <Button
      ...
      <DropdownMenuContent align="end">
        ...
        <DropdownMenuItem asChild>
          <Button
            variant="ghost"
            className="w-full justify-start px-2 py-0 flex items-center gap-2"
            onClick={() => authClient.signOut()} // sign out через better-auth
          >
            <LogOut className="h-4 w-4" />
            Log Out
          </Button>
